...
use engram::embeddings::{EmbeddingEngine, rrf_fuse};
...
"#;
    ...
    let affected_symbols: Vec<String> = cascade_result
        .log
        .iter()
...
"#;
    ...
    let r2 = parser
        .parse_source(
            "fn process() { /* data logic */ }",
            "rust",
            "data/processor.rs",
        )
        .unwrap();
    store
        .sync_file(Path::new("auth/processor.rs"), &r1)
        .unwrap();
    store
        .sync_file(Path::new("data/processor.rs"), &r2)
        .unwrap();
...
fn rrf_fusion_outperforms_or_matches_single_signals() {
    ...
    assert!(
        fused.iter().any(|r| r.symbol_id == "a"),
        "a should be in fused results"
    );
    assert!(
        fused.iter().any(|r| r.symbol_id == "c"),
        "c should be in fused results"
    );

    // E (only in signal2 at rank 4) should have lower score than B
    let b_score = fused
        .iter()
        .find(|r| r.symbol_id == "b")
        .map(|r| r.score)
        .unwrap_or(0.0);
    let e_score = fused
        .iter()
        .find(|r| r.symbol_id == "e")
        .map(|r| r.score)
        .unwrap_or(0.0);
    assert!(
        b_score > e_score,
        "b (multi-signal) should score higher than e (single-signal)"
    );
}
...
fn ndcg_worst_ranking() {
    ...
    assert!(
        ndcg < 0.5,
        "worst ranking should give low NDCG, got {}",
        ndcg
    );
}
...
fn ndcg_all_irrelevant() {
    ...
    assert!(
        (ndcg - 0.0).abs() < 1e-6,
        "all-irrelevant should give NDCG=0"
    );
}
...
fn scope_enriched_text_contains_structural_context() {
    ...
    assert!(
        enriched.contains("function validate_token"),
        "should contain kind + name"
    );
    assert!(
        enriched.contains("AuthService"),
        "should contain scope chain"
    );
    assert!(
        enriched.contains("pub fn validate_token"),
        "should contain signature"
    );
    assert!(
        enriched.contains("Validates JWT tokens"),
        "should contain docstring"
    );
    assert!(
        enriched.contains("handle_request"),
        "should contain callers"
    );
    assert!(
        enriched.contains("jsonwebtoken"),
